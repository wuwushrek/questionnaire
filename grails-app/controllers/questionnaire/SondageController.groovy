...
class SondageController {
	...
	static hasMany = [sondageAVenir:Sondage,sondageEnCours:Sondage,sondageTermine:Sondage]//???????????????
	...
	def tri(){
		...
		for(int i=0;i<Sondage.list().size;i++){
			if(current<Sondage.list().get(i).debut){
