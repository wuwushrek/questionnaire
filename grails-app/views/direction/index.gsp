		...
		<div id="list-direction" class="content scaffold-list" role="main">
			...
			<thead>
				...
				<g:checkAVenir sondage="$sondageInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						<td><g:link controller="sondage" action="edit" id="${sondageInstance}">${fieldValue(bean: sondageInstance, field: "nom")}</g:link></td>
						<td><g:link controller="sondage" action="edit" id="${sondageInstance}">${fieldValue(bean: sondageInstance, field: "moy")}</g:link></td>
					</tr>
				</g:checkAVenir>
				</g:each>
				</tbody>
